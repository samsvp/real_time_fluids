...
int main()
{
    ...
    // important: always use set to edge
    ...
    unsigned char *data = stbi_load(std::filesystem::path("textures/black.png").c_str(), 
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load(std::filesystem::path("textures/black.png").c_str(), 
        &width, &height, &nrChannels, 0);
    if (data)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        advectShader.setFloat("sigma_x_inv", 1 / s_width);
        advectShader.setVec2("u_resolution", s_width, s_height);
